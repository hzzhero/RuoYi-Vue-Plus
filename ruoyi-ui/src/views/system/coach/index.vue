    ...
    <el-row :gutter="10" class="mb8">
      ...
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['system:coach:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          @click="handleExport"
          v-hasPermi="['system:coach:export']"
        >导出</el-button>
      </el-col>
...
<script>
import { listCoach, getCoach, delCoach, addCoach, updateCoach } from "@/api/system/coach";
...
export default {
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$confirm('是否确认删除VIEW编号为"' + userIds + '"的数据项?', "警告", {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
          type: "warning"
        }).then(() => {
          this.loading = true;
          return delCoach(userIds);
        }).then(() => {
          this.loading = false;
          this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      downLoadExcel('/system/coach/export', this.queryParams);
    }
  }
};
</script>
